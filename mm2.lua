...
-- ===== РУЧНОЙ ДРАГ (работает на телефоне) =====
...
-- Открытие меню по тапу (короткий тап без движения)
local lastTapTime = 0
local tapStart = nil
iconFrame.InputBegan:Connect(function(input)
...
iconFrame.InputEnded:Connect(function(input)
    if (input.UserInputType == Enum.UserInputType.Touch 
    or input.UserInputType == Enum.UserInputType.MouseButton1) and tapStart then
        local tapDuration = tick() - tapStart
        if tapDuration < 0.3 then
...
contentArea.Size = UDim2.new(1, -145, 1, -72)
...
listTitle.Text = "Выбрать цель:"
listTitle.TextColor3 = Color3.fromRGB(255, 200, 0)
...
playerList.Size = UDim2.new(1, -20, 1, -30)
playerList.Position = UDim2.new(0, 10, 0, 26)
