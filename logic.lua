--[[
    Blocks Farm V20 | Part 2 - LOGIC
    + Auto Pickup FIX (3 способа + телепорт)
--]]
...
local function tryPickup(obj, hrp, range)
    ...
    -- Способ 3: Телепорт к предмету (авто-подбор при касании)
    if d < range then
...
RunService.Heartbeat:Connect(function()
    ...
    -- Noclip (проход через стены во время Auto Farm)
        ...
        if mob then
            ...
            if mh then
                if CFG.FarmMode == "TP" then
                    ...
                    if distance > 1 then
                        local flySpeed = CFG.FarmSpeed / 30
    ...
    -- Auto Pickup (FIX: GetDescendants + range 100)
