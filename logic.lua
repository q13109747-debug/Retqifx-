--[[
    Blocks Farm V24 | Part 2 - LOGIC
    Без анти-фола (не кикает) + Auto Farm + Fly в моба + Auto Pickup
--]]
...
RunService.Heartbeat:Connect(function()
    ...
    -- Auto Farm (БЕЗ ТЕЛЕПОРТА ВВЕРХ, БЕЗ КИКА)
        ...
        if mob then
            ...
            if mh then
                if CFG.FarmMode == "TP" then
                    -- TP: точно к мобу (не выше)
                    hrp.CFrame = mh.CFrame
                else
                    -- Fly: плавно к мобу (без ТП наверх)
                    local targetPos = mh.Position
                    local distance = (hrp.Position - targetPos).Magnitude
                    if distance > 3 then
                        if distance > 100 then
                            -- Далеко — телепорт к мобу (не наверх)
                            hrp.CFrame = mh.CFrame
                        else
