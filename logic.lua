--[[
    Blocks Farm V22 | Part 2 - LOGIC
    + Anti-fall (не падает под карту) + Fly в моба + Auto Pickup fix
--]]
    ...
    for _, mob in ipairs(workspace:GetDescendants()) do
        if mob:IsA("Model") and mob:FindFirstChildOfClass("Humanoid") then
            if names[mob.Name] 
               ...
               and isMobName(mob.Name)
               and mob.Name ~= "Boss Rune" then
                local h = mob:FindFirstChildOfClass("Humanoid")
                if h.Health > 0 then
                    local mh = mob:FindFirstChild("HumanoidRootPart") or mob:FindFirstChild("Torso")
                    if mh then
                        -- Проверка Y: моб не под картой
                        if mh.Position.Y > -10 then
...
local function findEnemy(range)
    ...
    for _, obj in ipairs(workspace:GetDescendants()) do
        if obj:IsA("Model") and obj:FindFirstChildOfClass("Humanoid") then
            if obj ~= char and isMobName(obj.Name) and obj.Name ~= "Boss Rune" then
...
local function isRune(name) 
    return matches(name, {"rune", "руна", "fragment", "фрагмент"}) 
end
...
local function tryPickup(obj, hrp, range)
    ...
    -- Телепорт только если предмет высоко
    if d < range and pos.Y > 5 then
...
RunService.Heartbeat:Connect(function()
    ...
    -- ANTI-FALL: если упал под карту — телепорт наверх
    if now - lastAntiFall > 1 then
        lastAntiFall = now
    ...
    if CFG.KillAura and now - lastKillAura > 0.2 then
        ...
        if tool then
            for _, mob in ipairs(workspace:GetDescendants()) do
                if mob:IsA("Model") and mob:FindFirstChildOfClass("Humanoid") then
                    if mob ~= char and isMobName(mob.Name) and mob.Name ~= "Boss Rune" then
                        local h = mob:FindFirstChildOfClass("Humanoid")
                        if h.Health > 0 then
                            local mh = mob:FindFirstChild("HumanoidRootPart") or mob:FindFirstChild("Torso")
                            if mh then
    ...
    -- Noclip (только в воздухе — не проваливаешься сквозь пол)
    if CFG.FarmEnabled and now - lastNoclip > 0.1 then
        lastNoclip = now
        -- Только если персонаж в воздухе ИЛИ высоко
        if hrp.Position.Y > 5 or hum.FloorMaterial == Enum.Material.Air then
    ...
    -- Auto Farm (ТОЧНО в моба, с защитой от падения)
    if CFG.FarmEnabled then
        local mob = findMob()
        if mob then
            local mh = mob:FindFirstChild("HumanoidRootPart") or mob:FindFirstChild("Torso")
            if mh then
                local mobPos = mh.Position
                
                -- Проверка: моб выше карты
                if mobPos.Y > -5 then
                    if CFG.FarmMode == "TP" then
                        -- TP: точно в моба +1 stud
                        local targetCF = mh.CFrame + Vector3.new(0, 1, 0)
                        if targetCF.Position.Y > 5 then
                            hrp.CFrame = targetCF
                        end
                    else
                        -- Fly: быстро в моба +1 stud
                        local targetPos = mobPos + Vector3.new(0, 1, 0)
                        if targetPos.Y > 5 then
                            local distance = (hrp.Position - targetPos).Magnitude
                            if distance > 0.5 then
                                local flySpeed = CFG.FarmSpeed / 8
