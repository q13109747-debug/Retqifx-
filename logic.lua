--[[
    Blocks Farm V23 | Part 2 - LOGIC
    FIXED Auto Farm + Anti-fall + Auto Pickup
--]]
...
-- FIND MOB (ПРОСТОЙ ПОИСК БЕЗ ЛИШНИХ ПРОВЕРОК)
local function findMob()
    ...
    -- Собираем список разрешённых имён
    local names = {}
    for id, _ in pairs(SelectedMobs) do 
        names[id] = true 
    end
    
    -- Если список пуст — берём всех мобов из текущей локации
    if next(names) == nil then
        local loc = S.SelectedLocation or "Loc 1"
        local w = S.SelectedWorld or "World 3"
        if S.Worlds and S.Worlds[w] and S.Worlds[w][loc] then
            for _, mobName in ipairs(S.Worlds[w][loc]) do
                names[mobName] = true
            end
        end
    end
        ...
        if mob:IsA("Model") and mob:FindFirstChildOfClass("Humanoid") then
            ...
            if names[mob.Name] and not Players:GetPlayerFromCharacter(mob) then
                ...
                if h and h.Health > 0 then
                    local mh = mob:FindFirstChild("HumanoidRootPart") or mob:FindFirstChild("Torso") or mob.PrimaryPart
                    if mh then
...
local function findEnemy(range)
    ...
    for _, obj in ipairs(workspace:GetDescendants()) do
        if obj:IsA("Model") and obj:FindFirstChildOfClass("Humanoid") then
            if obj ~= char and obj.Name ~= "Boss Rune" and obj.Name:match("^%d_%d_%d$") then
...
RunService.Heartbeat:Connect(function()
    ...
    -- AUTO FARM (простой, работает всегда)
    if CFG.FarmEnabled and now - lastFarm > 0.05 then
        lastFarm = now
        local mob = findMob()
        if mob then
            local mh = mob:FindFirstChild("HumanoidRootPart") or mob:FindFirstChild("Torso") or mob.PrimaryPart
            if mh then
                if CFG.FarmMode == "TP" then
                    -- TP: телепорт прямо к мобу
                    hrp.CFrame = mh.CFrame + Vector3.new(0, 2, 0)
                else
                    -- Fly: летит к мобу быстро
                    local targetPos = mh.Position
                    local distance = (hrp.Position - targetPos).Magnitude
                    if distance > 3 then
                        if distance > 100 then
                            hrp.CFrame = mh.CFrame + Vector3.new(0, 3, 0)
                        else
